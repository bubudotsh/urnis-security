...
os_detections() {
    ...
    printf "%-20s %s\n" "OS NAME" "$(egrep '^(NAME)=' /etc/os-release | cut -c 7- | sed 's/"//g')" 2>&1 | tee -a data/audit
    printf "%-20s %s\n" "OS VERSION" "$(egrep '^(VERSION)=' /etc/os-release | cut -c 9- | sed 's/"//g')" 2>&1 | tee -a data/audit
...
clean () {
    sed -i -e's/\[0;37m//' data/audit
    sed -i -e's/\[0m//' data/audit
}
